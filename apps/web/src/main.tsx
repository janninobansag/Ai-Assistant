...
export function App() {
  ...
  const [answerMaterial, setAnswerMaterial] = useState<Material | null>(null);
  ...
  async function showAnswers(material: Material, question?: string) {
    setOpenMaterialMenuId("");
    setAnswerLoading(true);
    setQuestionAnswers(null);
    setAnswerMaterial(material);
    setError("");
    try {
      setQuestionAnswers(
          ...
          <section className="mt-8 rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 sm:p-6">
                ...
                {questionAnswers.answers.map((item, index) => (
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm text-slate-700">
                      <span className="font-semibold">Answer {index + 1}:</span> {item.answer}
                    </p>
                    {!item.supported && (
            ...
            {answerMaterial && (
              <form
                ...
                onSubmit={(event) => {
                  event.preventDefault();
                  const question = externalQuestion.trim();
                  if (question.length >= 6) void showAnswers(answerMaterial, question);
                }}
              >
                <label className="text-sm font-semibold text-slate-900" htmlFor="external-question">
                  Ask about this material
                </label>
                <p className="mt-1 text-xs text-slate-500">
                  You can ask a question that is not written in the notes. This uses one AI point.
                </p>
                <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                  <input
                    ...
                    maxLength={1000}
                    placeholder="Example: What is the main idea of this topic?"
                    className="min-w-0 flex-1 rounded-xl border border-slate-300 px-3 py-2 text-sm"
                  />
                  <button
                    type="submit"
                    disabled={externalQuestion.trim().length < 6 || answerLoading}
