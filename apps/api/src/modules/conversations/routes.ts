...
router.post("/conversations", async (req, res) => {
  ...
  if (!parsed.success)
    return res.status(400).json({
      error: {
        ...
        message: "Choose a subject and at least one material.",
        requestId: null
      }
    });
  ...
  const materials = await Material.find({
    _id: { $in: parsed.data.materialIds },
    userId: owner,
    subjectId: parsed.data.subjectId,
    archivedAt: { $exists: false }
  }).select("_id title");
  if (!subject || materials.length !== parsed.data.materialIds.length)
    return res.status(404).json({
      error: {
        code: "NOT_FOUND",
        message: "Selected study material was not found.",
        requestId: null
      }
    });
  const conversation = await Conversation.create({
    userId: owner,
    ...parsed.data,
    title: parsed.data.title ?? `Tutor: ${materials[0]!.title}`
  });
...
router.get("/conversations/:conversationId", async (req, res) => {
  ...
  if (!conversation)
    return res
      .status(404)
      .json({ error: { code: "NOT_FOUND", message: "Conversation not found.", requestId: null } });
  const messages = await Message.find({ conversationId: conversation.id, userId: userId(req) })
    .sort({ createdAt: -1 })
    .limit(RECENT_MESSAGE_LIMIT)
    .lean();
  return res.json({
    data: { ...item(conversation), messages: messages.reverse() },
    meta: { requestId: null }
  });
});
router.post("/conversations/:conversationId/messages", async (req, res) => {
  ...
  if (!parsed.success)
    return res.status(400).json({
      error: {
        code: "VALIDATION_ERROR",
        message: "Enter a question of up to 2,000 characters.",
        requestId: null
      }
    });
  const owner = userId(req);
  ...
  if (parsed.data.clientMessageId) {
    ...
    if (existing)
      return res.status(409).json({
        error: { code: "CONFLICT", message: "This message was already sent.", requestId: null }
      });
  }
  ...
  try {
    ...
    const sources = await retrieveChunks(
      owner,
      conversation.materialIds.map(String),
      parsed.data.content
    );
    let answer = "";
    const priorSummary = typeof conversation.summary === "string" ? conversation.summary : "";
    for await (const part of streamGroundedAnswer(
      parsed.data.content,
      sources,
      ...
      if (cancelled) break;
      answer += part;
      writeEvent(res, "delta", { text: part });
    }
    if (cancelled) {
      await Message.updateOne(
        { _id: assistant.id },
        { $set: { status: "interrupted", content: answer } }
      );
      return;
    }
    ...
    await Message.updateOne(
      { _id: assistant.id },
      { $set: { content: answer, citations, status: "completed" } }
    );
    try {
      ...
      await Conversation.updateOne(
        { _id: conversation.id, userId: owner },
        { $set: { lastMessageAt: new Date() } }
      );
    }
    writeEvent(res, "citations", { items: citations });
    writeEvent(res, "done", { usage: { points: 1 } });
    res.end();
  } catch (error) {
    if (cancelled) {
    ...
    await Message.updateOne({ _id: assistant.id }, { $set: { status: "failed" } });
    writeEvent(res, "error", {
      message: error instanceof Error ? error.message : "Tutor unavailable."
    });
    res.end();
  }
});
router.post("/conversations/:conversationId/archive", async (req, res) => {
  const found = await Conversation.findOneAndUpdate(
    { _id: req.params.conversationId, userId: userId(req), archivedAt: { $exists: false } },
    { $set: { archivedAt: new Date() } },
    { new: true }
  );
  return found
    ? res.json({ data: item(found), meta: { requestId: null } })
    : res.status(404).json({
        error: { code: "NOT_FOUND", message: "Conversation not found.", requestId: null }
      });
});
