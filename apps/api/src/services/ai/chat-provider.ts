...
export async function* streamGroundedAnswer(
  ...
  if (env.AI_PROVIDER === "fake") {
    yield sources.length
      ? `Based on ${sources[0]!.label}, ${sources[0]!.text.slice(0, 500)}`
      : "I could not find support for that answer in the selected material.";
    return;
  }
  if (!env.GEMINI_API_KEY) throw new Error("Hosted AI is not configured.");
  const response = await hostedFetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(env.HOSTED_AI_MODEL)}:streamGenerateContent?alt=sse`,
    {
      method: "POST",
      headers: { "content-type": "application/json", "x-goog-api-key": env.GEMINI_API_KEY },
      body: JSON.stringify({
        contents: [{ parts: [{ text: buildPrompt(question, sources, history, summary) }] }],
  ...
  while (true) {
    ...
    const { complete, remainder } = consumeEvents(buffer);
    buffer = remainder;
